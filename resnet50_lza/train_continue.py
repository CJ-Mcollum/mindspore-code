...
model_save_path = "./model/{}.ckpt".format("ground_solid_DL_50_0.96")
con_save_path = "./model/{}_continue.ckpt".format("ground_solid_DL")
num_epochs = 50
batch_size_num = 24
...
train_loss_path = "./eval_data/train_continue/train_loss.txt"
train_acc_path = "./eval_data/train_continue/train_acc.txt"
test_acc_path = "./eval_data/train_continue/test_acc.txt"
...
def train_continue_main(tvt_path, num_epochs, json_path, model_save_path, con_save_path, batch_size, device_target):
    # 设置使用设备，CPU/GPU/Ascend
    context.set_context(mode=context.GRAPH_MODE, device_target=device_target)
    ...
    # 加载预训练模型
    param_dict = load_checkpoint(model_save_path)
    ...
    eval_cb = EvalCallBack(apply_eval, eval_param_dict, infer_param_dict, num_epochs, besk_ckpt_name=con_save_path)

    print("\n* continue training start\n")
    # 模型训练
    model.train(num_epochs, train_ds, callbacks=[eval_cb, TimeMonitor()], dataset_sink_mode=False)
    print()
    print("Continue training has been completed")
...
if __name__ == '__main__':
    train_continue_main(tvt_path, num_epochs, json_path, model_save_path, con_save_path, batch_size_num, device_target)